...
class Data_Analysis(object):
    ...
    def Extraction(self):
        ...
        Differential.remove(max(Differential[20:90]))
        Differential.remove(max(Differential[20:90]))
        Differential.remove(max(Differential[20:90]))
        Differential.remove(max(Differential[20:90]))
        #可能存在奇点，故删掉四个点
        Mobility = (max(Differential[20:90])**2)*2*IGZO_L/(IGZO_W*IGZO_GiCap)
...
class Shortstep_Data_Analysis(object):
    def __init__(self, fname):
        self.fname = fname
    ...
    def TransferExcel(self):
        Data = xlrd.open_workbook(self.fname)
        # Transfer_table = Data.sheet_by_name(u"Data")
        Transfer_table = Data.sheets()[0]
    #     Transfer_table = Data.sheet_by_index(1) #三种获取表单的方法。对EXCEL文件中的第二张表进行处理
    #   print Transfer_table.nrows, Transfer_table.ncols
        Id_TestValue = Transfer_table.col_values(1)  #表中第三列为Id值
        Vg_Value = Transfer_table.col_values(0)   #表中第二列为Vg值
        Vg = []
        for ele in Vg_Value:
            if isinstance(ele, float):
                Vg.append(ele)
    ...
    def Extraction(self):
        ...
        #提取开关电流比Ion/Ioff参数:
        Ioff = sum(Id_AbsValue[5:10])/5
        Ion = sum(Id_AbsValue[-10:-5])/5
        Ion_off = int(Ion/Ioff)
        Ion_off = '%E' %(Ion_off)
        Rate_OnOff = Ion_off
        # print 'The rate of On/Off: %e' %Rate_OnOff
        ...
        for i in range(1,len(Vg)):
            Differential.append((Id_SqrtValue[i]-Id_SqrtValue[i-1])/(Vg[i]-Vg[i-1]))
        Differential.remove(max(Differential[20:(len(Vg)-10)]))
        Differential.remove(max(Differential[20:(len(Vg)-10)]))
        Differential.remove(max(Differential[20:(len(Vg)-10)]))
        Differential.remove(max(Differential[20:(len(Vg)-10)]))
        #可能存在奇点，故删掉四个点
        Mobility = (max(Differential[20:(len(Vg)-10)])**2)*2*IGZO_L/(IGZO_W*IGZO_GiCap)
        Mobility = '%.2f' %Mobility# print 'The mobility is :', Mobility
...
if __name__ == '__main__':
    #参数提取测试,针对607实验室，步进为0.5。在‘’中填写文件名即可。
    # Trans11 = Data_Analysis('N2_60min.xls')
    # print Trans11.Extraction()

    #小步进测试，针对净化间4200，任意步进。在‘’中填写文件名即可。
    Trans = Shortstep_Data_Analysis('4200TEST.xls')
    print Trans.Extraction()
...
#for i in range(121):
#    NewTable.write(i, 1, Id_AbsValue)
